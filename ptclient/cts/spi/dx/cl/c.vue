  ...
  <div>
    <el-form>
      <el-form-item>
        ...
        ></el-input>
        <!-- setRemDescInVstOnDelay -> Full form: Set diagnosis description in view state on delay -->
      </el-form-item>
      <el-form-item>
    ...
    getRemDescUsingCache() {
      ...
      if (this.ORMRowIDForPreviousInvocation === this.firstParam) {
        ...
        if (!arEditRowsFromORM.length) {
          ...
          this.addEmptyRemToUI(arFromORM.dxName)
        }
      }
    ...
    async sendDataToServer() {
      try {
        const response = await fetch(`${DIAGNOSIS_API_URL}/${this.uuid}`, {
          ...
          body: JSON.stringify({
            dxName: this.getRemDescUsingCache(),
          }),
        })
        if (!response.ok) {
