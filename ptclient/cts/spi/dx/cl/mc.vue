...
Performance:
Inside dx/cl/c.vue:getDxDescUsingCache enable the console.log 
console.log(
        'Inside get desc. Only first time it needs to come from ORM and subsequently it can always come from cache, the value set by setDxDescOn5KeyPress'
      )
When this Ct is mounted it loads the changeDx 204 times since the above console.log is put on console 204 times. 

Every time the slide is changed the getDxDescUsingCache() is again called 204 times since on console.log I see 204 messages
...
3. getArrayOfDxIDsToShowInThisCard depends on this.diVirtualSlideNumber
    ...
    <el-carousel arrow="always" trigger="click" :autoplay="false" @change="slideChanged">
      ...
      <el-carousel-item v-for="slide in getNumOfCarouselSlides" :key="slide">
        ...
        <el-row type="flex" :gutter="20">
          <el-col v-for="dxID in getArrayOfDxIDsToShowInThisCard" :key="dxID">
            <el-card>
              <changeDx :first-param="dxID"></changeDx>
            </el-card>
          </el-col>
        </el-row>
      </el-carousel-item>
...
import changeDx from '@/cts/spi/dx/cl/c.vue'
export default {
  components: { changeDx },
  data() {
    return {
  ...
  computed: {
    ...
    getArrayOfDxIDsToShowInThisCard() {
      ...
      console.log('First dx card', firstCard)
  ...
  mounted() {
    ...
    const resultArFromORM = ormDx.query().get()
