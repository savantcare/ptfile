...
<template>
  <div>
    ...
    >
      <el-table-column prop="dxName" label="Diagnoses sending to server"></el-table-column>
    </el-table>
    ...
    >
      <el-table-column
        prop="dxName"
        label="Error: Diagnoses attempted but failed to save"
      ></el-table-column>
    </el-table>
  </div>
</template>
<script>
import ormDx from '../db/vuex-orm/dx.js' // Path without @ can be resolved by vsCode. Hence do not use webpack specific @ sign that represents src folder.
...
export default {
  ...
  computed: {
    cfGetOrmEditStateRows() {
      return ormDx.getNewRowsInEditState()
    },
    ...
    cfGetOrmApiSendingStateRows() {
      return ormDx.getApiSendingStateRows()
    },
  },
  ...
  methods: {
    ...
    mfSetFieldInOrmOnTimeout(pEvent, pOrmRowId, pFieldName) {
      const rowStatus = 24
      ormDx.setField(pEvent, pOrmRowId, pFieldName, rowStatus)
